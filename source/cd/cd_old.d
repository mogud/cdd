...
void cdPutImageRectMap(int iw, int ih, const(ubyte) * index, const(long) * colors, int x, int y, int w, int h, int xmin, int xmax, int ymin, int ymax);
...
void cdPutImageRGBA(int iw, int ih, const(ubyte) * r, const(ubyte) * g, const(ubyte) * b, const(ubyte) * a, int x, int y, int w, int h) {
    cdPutImageRectRGBA((iw), (ih), (r), (g), (b), (a), (x), (y), (w), (h), 0, 0, 0, 0);
}

void cdPutImageMap(int iw, int ih, const(ubyte) * index, const(long) * colors, int x, int y, int w, int h) {
    cdPutImageRectMap((iw), (ih), (index), (colors), (x), (y), (w), (h), 0, 0, 0, 0);
}

void cdPutImage(cdImage* image, int x, int y) {
    cdPutImageRect((image), (x), (y), 0, 0, 0, 0);
}
...
pragma(inline, true)
void cdCanvas2Raster(T)(T x, int *y) {
    cdUpdateYAxis(y);
}
